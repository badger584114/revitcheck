...
- **Revision clouds/triangles** — drawn on the sheet body: a closed scalloped-curve entity (the cloud) paired with a small triangle/delta symbol carrying the revision tag. **Not yet built — paused as of 2026-08-09 pending sample drawings that actually contain revision clouds** (the current sample's every sheet has a single clean revision, `AMEND No. 0`, nothing to calibrate detection against; see CLAUDE.md's implementation-status note). Worth correcting an assumption this paragraph originally made: it isn't safe to assume triangle+tag detection will "mirror" the cross-sheet marker detection above, because that turned out to be text-adjacency-based (a tag stacked on a sheet number), not shape-based — see extraction/references.py's docstring. A cloud's scalloped-curve outline genuinely is a distinctive vector shape with no equivalent text signal, so cloud detection likely does need real shape-based heuristics (arc-count/curvature on the closed path) that the marker work ended up not needing; the triangle+tag pairing might still reuse a text-adjacency approach once real geometry is in front of us to check. Block/attribute lookup remains the plan if DXF support is ever added (§1).
