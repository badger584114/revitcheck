...
*General note references* (free text, e.g. "see Detail 4 on Dwg S-201") remain out of scope for now, same deferral as originally planned — not attempted by the current extraction, which instead actively guards against misreading this kind of sentence as a view title (a body-text word like "REFER"/"SEE" immediately before "SECTION"/"DETAIL" excludes the match; see the module's docstring).
