...
2. **A per-sheet DXF-space -> PDF-page-space transform**, to map a geometry Issue's location onto the right spot on the PDF page. Confirmed low-risk: **this firm's workflow is Revit-authored, with each sheet view exported to both PDF and DWG from the same source** (user-confirmed, 2026-08-10) — so the two should already agree on scale/extents per sheet, meaning the transform is a derivable affine mapping from each sheet's plot scale, not a georeferencing problem. Still needs confirming against a real converted-DXF-next-to-its-PDF-page pair once DXF ingestion exists (§9 build order) — the first concrete thing to check there, before building the general transform.
...
3. Geometry checks: dimensional consistency within a single sheet (start here) before attempting full multi-sheet structure reconstruction — **requires DXF ingestion** (§1): a real build attempt confirmed PDF-only dimension-line reconstruction is unreliable on repeated/stacked dimensions, so this step now includes bringing up `ezdxf` parsing (DIMENSION entities, block/insert attributes, layer table) scoped to what geometry checks need, not deferred the way it is for drafting
