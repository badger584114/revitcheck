...
3. Geometry checks: dimensional consistency within a single sheet (start here) before attempting full multi-sheet structure reconstruction — **requires DXF ingestion** (§1): a real build attempt confirmed PDF-only dimension-line reconstruction is unreliable on repeated/stacked dimensions, so this step now includes bringing up `ezdxf` parsing scoped to what geometry checks need, not deferred the way it is for drafting. **DXF ingestion itself done (2026-08-10)** — `extraction/dxf_source.py` (`convert_dwg_to_dxf` via ODA File Converter as a local subprocess, `ingest_dxf` via `ezdxf`) extracts `DIMENSION` and `VIEWPORT` entities into `ir.py`'s new `DxfSheet`, calibrated against all 31 real files in `samples/dwg/` — see that module's docstring. Block-insert attributes turned out not to exist on this firm's export (§4), so title-block/marker extraction from DXF isn't built yet — not needed by this step, which only needs dimensions + units. **Still to build: the actual check rule** (§5a's drawn-vs-stated comparison using the rounding-grid tolerance) — ingestion produces the input, nothing consumes it yet
