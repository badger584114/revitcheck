...
public static class CheckRegistry
{
    ...
    /// <summary>Registers the ported dimension checks (revit.dimension_provenance, revit.dimension_override_consistency) plus revitcheck.pile_model_schedule_consistency, revitcheck.pile_chain_bearing_consistency and revitcheck.spot_elevation_consistency, bound to a RuleConfig for this run.</summary>
    public static void RegisterAll(Catalog catalog, RuleConfig ruleConfig)
    {
        ...
        catalog.Register(DrawnDimensionConsistencyCheck.RuleId, model => DrawnDimensionConsistencyCheck.Run(model, ruleConfig));
    }
}
