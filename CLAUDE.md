...
| `revitcheck.abutment_elevation_consistency` | Native add-in, built 2026-09-02 (PLANNING.md §18) — the second element-type check, and the first to verify against raw solid geometry rather than a schedule or a live parameter. Compares a Spot Elevation's own drafted value (`DimensionInfo.Origin.Z` — `Value`/`ValueOverride` confirmed unconditionally null for this family) against real horizontal `PlanarFace`s found near it via `Face.Project`, judged by 2D (plan) proximity, never by Z agreement (picking whichever face happens to agree would be circular). Deliberately **not** filtered by category anywhere — real data confirmed a category-filtered search (the first cut, `OST_StructuralFraming`) "will fall over as soon as we put another model into it": Structural Framing is an old, being-phased-out modelling workflow on this project specifically, not even stable within one client's own project history, let alone across clients. Core-side built and tested (9 new tests, 341 total); Addin geometry-walk work ported line-for-line from a real, validated diagnostic (`InspectDimensionGeometry.pushbutton`, extended the same day) that matched 3 real Spot Elevations to within a few millimetres (two exact). Standalone ribbon button only, not dual-mode yet — matches this codebase's own precedent (both pile buttons started standalone before Stage 3 added session integration). **Not yet run on the Revit machine.** See PLANNING.md §18 and native/README.md. |
...
- ~~**Abutment beam placement**~~ — **built 2026-09-02 as `revitcheck.abutment_elevation_consistency`** (PLANNING.md §18, see the Built state table above). Diagnostic-first discipline paid off exactly as it did for piles: real ground truth (a Spot Elevation's `Value` is unconditionally null; its `Reference` and nearby parameters are both unreliable; a category-filtered geometry search "will fall over as soon as we put another model into it") shaped a real check built only after 3 real samples matched real solid geometry to within a few millimetres. **Not yet run on the Revit machine** — the next real validation step, same discipline as everything else in this document.
