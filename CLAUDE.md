...
pdfchecker/
  src/pdfchecker/
    ...
    extraction/
      ...
      dxf_source.py                 # Stage 3 (§5): convert_dwg_to_dxf (ODA File Converter,
                                    # local subprocess) + ingest_dxf(path) -> DxfSheet — see its
                                    # docstring for what real converted DXF confirmed vs.
                                    # corrected from PLANNING.md's original assumptions
    checks/                        # Stage 2: the drafting check engine (§4)
      ...
      geometry.py                   # Stage 3 (§5a): drawn-vs-stated dimension_consistency rule —
                                    # consumes Sheet.dxf_sheet, scoped to linear dimensions with
                                    # a numeric override (see its docstring for why)
  config/
  ...
  tests/
    ...
    test_geometry.py               # Stage 3 (§5a): synthetic tolerance-logic branches + a real-
                                    # sample assertion via samples/dxf/101051 (attached through the
                                    # real attach_dxf_sheets join, not hand-wired)
